...
public:
    ...
    ~persistent_string()
        requires(!std::is_trivially_destructible_v<char_t>)
    {
        for (size_tt i; i < len; i++) {
            chars[i].~char_t();
        }
...
class persistent_string_store {
    using char_t = char;
    using param_char_t = std::conditional_t<
        std::is_trivially_copyable_v<char_t> && sizeof(char_t) <= sizeof(size_tt), char_t, const char_t&>;
...
public:
    class string_store_iterator {
        ...
        string_store_iterator(const std::vector<std::byte*>& container, const size_tt& byte_size)
            : container(container), string(reinterpret_cast<persistent_string<char_t>*>(container.front())),
              byte_size(byte_size), chunk_index(0) {}
    ...
    public:
        auto operator++() -> string_store_iterator& {
            ...
            const void* next_address = std::align(alignof(persistent_string<char_t>), 0, ptr, space);
    ...
    auto start_recording(size_tt min_num_chars = 0) -> bool {
        ...
        size_tt min_required_space =
            alignment_space + sizeof(persistent_string<char_t>) + min_num_chars * sizeof(char_t);
    ...
    auto recordChar(param_char_t c) -> bool {
        ...
        MY_ASSERT(
            size_bytes ==
            reinterpret_cast<std::byte*>(&current_recording->chars[current_recording->len]) - memory.back()
        );
        return true;
    }
    ...
    auto recordString(const std::basic_string_view<char_t>& string) -> bool {
        ...
        if constexpr (std::is_trivially_copyable_v<char_t>) {
            std::memcpy(
                &current_recording->chars[current_recording->len], string.data(), sizeof(char_t) * string.size()
            );
