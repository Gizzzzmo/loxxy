...
concept IsSimpleTypeFunction = requires() { typename T::template type<In>; };
...
concept IsSimpleResolvingIndirection =
    IsSimpleTypeFunction<T, In> && requires(T::template type<In> mapped, Res& res, const Res& c_res) {
...
concept IsIndirection = (same_as<Res, void> && (IsSimpleIndirection<T, Ins> && ...)) ||
