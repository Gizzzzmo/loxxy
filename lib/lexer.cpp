...
static CompileTimeInit getInitialIdStore() {
    ...
    const char* keywords[]{"and", "class", "else",   "false", "for",  "fun",  "if",  "nil",
    ...
    while (space_per_keyword < max_len + sizeof(persistent_string<char>) ||
           space_per_keyword < alignof(persistent_string<char>)) {
        space_per_keyword <<= 1;
...
enum LiteralFormat { DEC, HEX, BIN, OCT };
...
public:
    ...
    bool scanTokens() {
        ...
        if (file.eof())
            addToken(END_OF_FILE, reinterpret_cast<persistent_string<char>*>(init.start_simple));
    ...
    bool scanTokensLine() {
        ...
        while (!file.eof() && !file.fail() && !done) {
            scanToken();
            if (last_line != line)
                addToken(NEW_LINE, reinterpret_cast<persistent_string<char>*>(init.start_simple));
...
private:
    ...
    void addToken(TokenType type, const persistent_string<char>* lexeme, Literal literal = Literal()) {
    ...
    void addIntegralLiteral(char start) {
        ...
        std::string_view number_string(&lexeme->chars[number_start], number_end - number_start);
    ...
    void scanToken() {
        ...
        case '\t':
            break;
        case '{':
            addToken(LEFT_BRACE, reinterpret_cast<persistent_string<char>*>(&init.start_simple[c * init.simple_space]));
            break;
        case '}':
            addToken(
                RIGHT_BRACE, reinterpret_cast<persistent_string<char>*>(&init.start_simple[c * init.simple_space])
            );
            break;
        case '(':
            addToken(LEFT_PAREN, reinterpret_cast<persistent_string<char>*>(&init.start_simple[c * init.simple_space]));
            break;
        case ')':
            addToken(
                RIGHT_PAREN, reinterpret_cast<persistent_string<char>*>(&init.start_simple[c * init.simple_space])
            );
            break;
        case ',':
            addToken(COMMA, reinterpret_cast<persistent_string<char>*>(&init.start_simple[c * init.simple_space]));
            break;
        case '.':
            addToken(DOT, reinterpret_cast<persistent_string<char>*>(&init.start_simple[c * init.simple_space]));
            break;
        case '-':
            addToken(MINUS, reinterpret_cast<persistent_string<char>*>(&init.start_simple[c * init.simple_space]));
            break;
        case '+':
            addToken(PLUS, reinterpret_cast<persistent_string<char>*>(&init.start_simple[c * init.simple_space]));
            break;
        case ';':
            addToken(SEMICOLON, reinterpret_cast<persistent_string<char>*>(&init.start_simple[c * init.simple_space]));
            break;
        case '/':
            if (match('/')) {
                std::string comment;
            ...
            addToken(SLASH, reinterpret_cast<persistent_string<char>*>(&init.start_simple[c * init.simple_space]));
            break;
        case '*':
            addToken(STAR, reinterpret_cast<persistent_string<char>*>(&init.start_simple[c * init.simple_space]));
            break;
        case '!':
            if (match('='))
                addToken(BANG_EQUAL, reinterpret_cast<persistent_string<char>*>(init.bang_equal));
            else
                addToken(BANG, reinterpret_cast<persistent_string<char>*>(&init.start_simple[c * init.simple_space]));
            break;
        case '=':
            if (match('='))
                addToken(EQUAL_EQUAL, reinterpret_cast<persistent_string<char>*>(init.equal_equal));
            else
                addToken(EQUAL, reinterpret_cast<persistent_string<char>*>(&init.start_simple[c * init.simple_space]));
            break;
        case '<':
            if (match('='))
                addToken(LESS_EQUAL, reinterpret_cast<persistent_string<char>*>(init.less_equal));
            else
                addToken(LESS, reinterpret_cast<persistent_string<char>*>(&init.start_simple[c * init.simple_space]));
            break;
        case '>':
            if (match('='))
                addToken(GREATER_EQUAL, reinterpret_cast<persistent_string<char>*>(init.greater_equal));
            else
                addToken(
                    GREATER, reinterpret_cast<persistent_string<char>*>(&init.start_simple[c * init.simple_space])
                );
            break;
        case '"':
            addStringLiteral();
