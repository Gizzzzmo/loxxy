...
struct Extractor
    : public IndirectVisitor<Extractor<Payload, Indirection, ptr_variant, Resolver>, Resolver, Indirection> {
...
auto& payload_of(ExprPointer<Payload, Indirection, ptr_variant>& node, Resolver&& resolver) {
    return visit(Extractor<Payload, Indirection, ptr_variant, Resolver>{std::forward<Resolver>(resolver)}, node);
}
...
const Payload& payload_of(const ExprPointer<Payload, Indirection, ptr_variant>& node, Resolver&& resolver) {
    return const_cast<const Payload&>(
        payload_of(const_cast<ExprPointer<Payload, Indirection, ptr_variant>&>(node), std::forward<Resolver>(resolver))
    );
}
