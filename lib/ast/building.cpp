...
concept NodeBuilder =
    std::same_as<Payload, typename T::Payload> && std::same_as<Indirection, typename T::Indirection> &&
    (ptr_variant == T::ptr_variant) &&
    requires(
        T t, ExprPointer<Payload, Indirection, ptr_variant>&& node, Token token, double x, bool b,
        const persistent_string<>* str, marker<BinaryExpr<Payload, Indirection, ptr_variant>> binary_expr,
        ...
        {
            t(binary_expr, std::move(node), std::move(node), token)
        } -> convertible_to<ExprPointer<Payload, Indirection, ptr_variant>>;
        { t(unary_expr, std::move(node), token) } -> convertible_to<ExprPointer<Payload, Indirection, ptr_variant>>;
        { t(grouping_expr, std::move(node)) } -> convertible_to<ExprPointer<Payload, Indirection, ptr_variant>>;
...
concept NodeCopier = std::same_as<Payload, typename T::Payload> && std::same_as<Indirection, typename T::Indirection> &&
                     ...
                     requires(
                         T t, ExprPointer<Payload, Indirection, ptr_variant>&& node, Token token, double x, bool b,
                         const persistent_string<>* str, const Payload& payload,
                         ...
                         marker<BoolExpr<Payload, Indirection, ptr_variant>> bool_expr,
                         marker<NilExpr<Payload, Indirection, ptr_variant>> nil_expr,
                         marker<ExpressionStmt<>> expr_stmt, marker<PrintStmt<>> print_stmt
                     ) {
                         {
                             t(binary_expr, payload, std::move(node), std::move(node), token)
                         } -> convertible_to<ExprPointer<Payload, Indirection, ptr_variant>>;
                         {
                             t(unary_expr, payload, std::move(node), token)
                         } -> convertible_to<ExprPointer<Payload, Indirection, ptr_variant>>;
                         {
                             t(grouping_expr, payload, std::move(node))
                         } -> convertible_to<ExprPointer<Payload, Indirection, ptr_variant>>;
                         {
                             t(string_expr, payload, str)
                         } -> convertible_to<ExprPointer<Payload, Indirection, ptr_variant>>;
                         {
                             t(number_expr, payload, x)
                         } -> convertible_to<ExprPointer<Payload, Indirection, ptr_variant>>;
                         ...
                         {
                             t(expr_stmt, std::move(node))
                         } -> convertible_to<StmtPointer<Payload, Indirection, ptr_variant>>;
                         {
                             t(print_stmt, std::move(node))
                         } -> convertible_to<StmtPointer<Payload, Indirection, ptr_variant>>;
                     };
...
concept PayloadBuilder = requires(
    ...
    marker<StringExpr<Payload, Indirection, ptr_variant>> string_expr,
    marker<BoolExpr<Payload, Indirection, ptr_variant>> bool_expr,
    marker<NilExpr<Payload, Indirection, ptr_variant>> nil_expr, marker<ExpressionStmt<>> expr_stmt,
...
struct DefaultPayloadBuilder {
    ...
    constexpr Payload operator()(Args&&...) const {
        return Payload{};
    }
};
